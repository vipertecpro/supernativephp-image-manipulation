{{--
    Story example — a locked 9:16 vertical crop, WhatsApp/Instagram-story
    style, with colour adjust. See App\NativeComponents\StoryPhoto.
--}}
<column class="w-full h-full bg-[#121417] safe-area">
    ...
    <row class="w-full items-center px-4 py-3">
        ...
        <text class="flex-1 text-center text-base font-semibold text-white">Story</text>
    ...
    <column class="w-full flex-1 items-center justify-center gap-6 px-5">
        @if ($sourcePath)
            @php([$pw, $ph] = $this->previewBox(240, 430))
            <column class="w-[{{ $pw }}] h-[{{ $ph }}] rounded-2xl overflow-hidden">
                <image src="{{ $sourcePath }}" :fit="1" alt="Story" class="w-full h-full" />
            </column>
        @else
        ...
        <text class="text-center text-sm text-white/50">
            {{ $sourcePath ? 'Tap Edit to reframe your story' : 'Add a photo for your story' }}
        </text>
    </column>
